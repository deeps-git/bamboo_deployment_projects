...
export bamboo_build_key; export bamboo_gitrepo_name; export bamboo_maven_path; export bamboo_function_ext; export bamboo_repo_url; export bamboo_scrip_path; export PATH="/home/xdeploy/.pythonbrew/pythons/Python-2.7/bin:${PATH}";
...
if [[ "${bamboo_function_ext}" == "$dvar" ]]; then
        ...
        ./gradlew -C rebuild clean buildall uploadArchives -i
fi
...
do
    ...
    wget --no-proxy "http://maven.teamccp.com/content/groups/master/${bamboo_maven_path}/${element}/${snapversion}/${element}-${snapversion}.zip" -O /tmp/${element}-${snapversion}.zip &&
...
#           zip -r -qq /tmp/${element}-${snapversion}.zip /tmp/unzipf
    ...
    if [ -d /tmp/unzipf/config ] && [ "${bamboo_function_ext}" == "$dvar" ]; then
        ...
        swapfunc dev-config.json application-config.json
        swapfunc dependency-config-dev.json dependency-config.json
    else
        if [ -d /tmp/unzipf/config ]; then
        echo "The present working directory is:"; pwd;
        echo "Config folder is present"
        swapfunc ci-config.json application-config.json
        swapfunc dependency-config-ci.json dependency-config.json
        else
