...
do
    ...
    funcout=$((aws --profile svc-primary-xbo lambda get-function --function-name ${awsfunctionname}${bamboo_function_ext} --qualifier "\$LATEST") 2>&1)
    errstrings='ResourceNotFoundException'
    if [[ $funcout != *"$errstrings"* ]]; then
        echo "Function has been found"
        aws --profile svc-primary-xbo lambda publish-version --function-name ${awsfunctionname}${bamboo_function_ext} --description "published version of ${piece} Latest to next version" > descr.txt
        cat descr.txt
        versionFromPublish=$(grep "Version" descr.txt | awk -F '[^0-9]*' '$0=$2')
        echo " "
        echo "The version number from the publish is $versionFromPublish"
        echo " "
        aliasdescr=$((aws --profile svc-primary-xbo lambda list-aliases --function-name ${awsfunctionname}${bamboo_function_ext}) 2>&1)
        matchalias='"Name": "QA"'
        if [[ $aliasdescr == *"$matchalias"* ]]; then
        echo "$piece contains alias with the name QA"
        echo "Updating $piece"
        echo $aliasdescr
        aws --profile svc-primary-xbo lambda update-alias --function-name ${awsfunctionname}${bamboo_function_ext} --name QA --function-version "${versionFromPublish}"
        else
        echo "$piece contains no alias named QA"
        echo "Creating a new alias named QA for $piece"
        aws --profile svc-primary-xbo lambda create-alias --function-name ${awsfunctionname}${bamboo_function_ext} --name QA --function-version "${versionFromPublish}"
        fi        
    else
        echo " "
