...
echo ""
echo ""
aws --profile svc-primary-xbo lambda list-aliases --function-name ${project_input}
    aliasprod=$((aws --profile svc-primary-xbo lambda list-aliases --function-name ${project_input}) 2>&1)
    matchalias='"Name": "PROD"'
            ...
            aws --profile svc-primary-xbo lambda update-alias --function-name ${project_input} --name PROD --function-version "${version_number}"
            else
            echo "$project_input DOESN'T contain PROD alias"
            echo " "
            echo "Creating a PROD alias for $project_input now!"
            aws --profile svc-primary-xbo lambda create-alias --function-name ${project_input} --name PROD --function-version "${version_number}"
            fi          
    else
        echo " "
