...
do
    ...
    s3upload () {
        ...
        /usr/bin/aws s3 cp /tmp/${element}-${version}.zip s3://crpl-lambda-functions-${region_array_element}/code/${bamboo_gitrepo_name}/${element}/$version/ --region ${region_array_element} --grants full=uri=http://acs.amazonaws.com/groups/global/AllUsers
    }
    cleaning () {
...
#       sudo rm -rf /tmp/unzipf/config/*.gpg
    ...
    cat /home/xdeploy/bamboo-agent-home/xml-data/build-dir/${bamboo_build_key}/${element}/gradle.properties > gradleprop.txt
        ...
        if [ -n "${awshandler// }" ]; then
            ...
            aws lambda update-function-code --region ${region_array_element} --function-name ${awsfunctionname}${bamboo_function_ext} --s3-bucket crpl-lambda-functions-${region_array_element} --s3-key code/${bamboo_gitrepo_name}/${element}/$version/${element}-${version}.zip
