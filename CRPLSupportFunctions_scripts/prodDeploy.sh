...
declare -a region_array=("us-west-1" "us-east-1")
...
if [ -n "${project_input// }" ]; then
        echo
                ...
                exit;
                        ...
                        /usr/bin/aws s3 cp ${project_input}.zip s3://crpl-lambda-functions-${region_array_element}/release/${gitrepo_name}/${awsfname}/${latest_tag}/ --region ${region_array_element} --grants full=uri=http://acs.amazonaws.com/groups/global/AllUsers
                        ...
                        awscode=$"S3Bucket=crpl-lambda-functions-${region_array_element},S3Key=release/${gitrepo_name}/${awsfname}/${latest_tag}/${project_input}.zip"
                        ...
                        if [[ $funcoutput2 == *"$errstring"* ]]; then
                            ...
                            aws lambda update-function-code --region ${region_array_element} --function-name ${awsfunctionname} --s3-bucket crpl-lambda-functions-${region_array_element} --s3-key release/${gitrepo_name}/${awsfname}/${latest_tag}/${project_input}.zip
