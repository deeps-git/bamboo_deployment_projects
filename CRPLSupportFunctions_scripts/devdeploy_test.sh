...
echo "The extension is ${bamboo_function_ext}"; dvar="-Trial-Dev"; svar="-Stage"; snapversion="TRIAL-0-SNAPSHOT";
if [[ "${bamboo_function_ext}" == "$dvar" ]]; then
        echo "Because the extension is -Trial-Dev, declaring snapversion as TRIAL-0-SNAPSHOT!"
        snapversion="TRIAL-0-SNAPSHOT"
    else
...
echo "Checking out as commercial-gateway-trial again!"; git checkout commercial-gateway-trial;
...
#           zip -r -qq /tmp/${element}-${snapversion}.zip /tmp/unzipf
    ...
    s3upload () {
        ...
        /usr/bin/aws s3 cp /tmp/${element}-${snapversion}.zip s3://crpl-support-functions-${region_array_element}/code/${bamboo_gitrepo_name}/${element}/$snapversion/ --region ${region_array_element}
    }
    cleaning () {
...
#       sudo rm -rf /tmp/unzipf/config/*.gpg
    ...
    if [ -d /tmp/unzipf/config ] && [ "${bamboo_function_ext}" == "$dvar" ]; then
        echo "Bamboo extension is -Trial-Dev hence swapping dev-config with application-config & dependency-config-dev with dependency-config"
    ...
    cat /home/xdeploy/bamboo-agent-home/xml-data/build-dir/${bamboo_build_key}/${element}/gradle.properties > gradleprop.txt
        ...
        if [ -n "${awshandler// }" ]; then
            if [[ $funcoutput == *"$errstring"* ]]; then
                ...
                aws logs put-subscription-filter --region ${region_array_element} --log-group-name /aws/lambda/${awsfunctionname}${bamboo_function_ext} --filter-name /aws/lambda/${region_array_element}/${awsfunctionname}${bamboo_function_ext} --destination-arn arn:aws:kinesis:${region_array_element}:453246518757:stream/$kinesisStreamName${bamboo_function_ext} --filter-pattern "" --role-arn arn:aws:iam::453246518757:role/OneCloud/Kinesis-to-Cloudwatch
            else
            echo "Function found"
            aws lambda update-function-code --region ${region_array_element} --function-name ${awsfunctionname}${bamboo_function_ext} --s3-bucket crpl-support-functions-${region_array_element} --s3-key code/${bamboo_gitrepo_name}/${element}/$snapversion/${element}-${snapversion}.zip
